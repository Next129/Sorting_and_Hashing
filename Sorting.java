public class Sorting {
	 ...
	 */
	static void SelectionSort(int[] A) { // Ex 1 Complete the method SelectionSort
		for (int i = 0; i < A.length - 1; i++) {
			int minIndex = i; // Index of smallest remaining value.
			minIndex = findIndexSmallest(A, i, A.length);
			// Ex1.Complete this method. Note that the method swap is provided.
			swap(A,i,minIndex);
	 ...
	 * @param A The integer array to be sorted.
	 */
	static void InsertionSort(int[] A) { // Ex 2 Complete the method InsertionSort
		for (int i = 1; i < A.length; i++) {
			int temp = A[i];
			for (int j = i - 1; j >= 0 && A[j] > temp; j--) {
				// Ex2.Complete this method. Note the method swap is provided.
			swap(A,j,j+1);
			}
		}
	}
